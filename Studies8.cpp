...
SCSFExport scsf_PercentChangeSinceOpen(SCStudyInterfaceRef sc)
{
	...
	else
	{
		...
		PercentChange[sc.Index] = (CurrentValue - OpenValue) / OpenValue * Multiplier.GetFloat();
	}
}
...
SCSFExport scsf_PercentChangeSincePreviousClose(SCStudyInterfaceRef sc)
{
	...
	if (PreviousCloseValue == 0)
		...
		PercentChange[sc.Index] = (sc.BaseData[InputData.GetInputDataIndex()][sc.Index] - PreviousCloseValue) / PreviousCloseValue;
}
...
SCSFExport scsf_PivotPointsDaily(SCStudyInterfaceRef sc)
{
	...
	const int NUMBER_OF_STUDY_SUBGRAPHS = 29;
	...
	float fR0_5 = 0, fR1 = 0, fR1_5 = 0, fR2 = 0, fR2_5 = 0, fR3 = 0, fR4 = 0, fR3_5 = 0,  fR5 = 0, fR6 = 0, fR7 = 0, fR8 = 0, fR9 = 0, fR10 = 0;
	float fS0_5 = 0, fS1 = 0, fS1_5 = 0, fS2 = 0, fS2_5 = 0, fS3 = 0, fS4 = 0, fS3_5 = 0,  fS5 = 0, fS6 = 0, fS7 = 0, fS8 = 0, fS9 = 0, fS10 = 0;
...
SCSFExport scsf_PivotPointsVariablePeriod(SCStudyInterfaceRef sc)
{
	...
	for (int index = sc.UpdateStartIndex; index < sc.ArraySize+NumberOfForwardBars; index++ )
	{
		...
		ValidPivotPoint = 
			...
			fS1, fS1_5, fS2, fS2_5, fS3, fR3_5, fS3_5, fR4, fS4, fR5, fS5,
...
SCSFExport scsf_VolumeAtPriceThresholdAlertV2(SCStudyInterfaceRef sc)
{
	...
	SCInputRef DrawExtensionLines = sc.Input[3];
	SCInputRef PercentageThreshold = sc.Input[5];
	SCInputRef AdditionalVolumeThreshold = sc.Input[6];
	SCInputRef AllowZeroValueCompares = sc.Input[7];
	SCInputRef DivideByZeroAction = sc.Input[8];
	SCInputRef HighlightAdjacentAlertsGroupSize = sc.Input[9];
	...
	if (sc.SetDefaults)
	{
		...
		ComparisonMethod.SetCustomInputStrings("Bid Volume;Ask Volume;Total Volume;Number of Trades;Ask Volume Bid Volume Difference;Ask Volume Bid Volume Diagonal Difference;Ask Volume Bid Volume Ratio;Ask Volume Bid Volume Diagonal Ratio;Bid Volume and Ask Volume Separately");
		...
		PercentageThreshold.Name = "Percentage Threshold";
		PercentageThreshold.SetInt(150);
		
		AdditionalVolumeThreshold.Name = "Additional Volume Threshold";
		AdditionalVolumeThreshold.SetInt(100);

		AllowZeroValueCompares.Name = "Enable Zero Bid/Ask Compares";
		AllowZeroValueCompares.SetYesNo(0);

		DivideByZeroAction.Name = "Zero Value Compare Action";
		DivideByZeroAction.SetCustomInputStrings("Set 0 to 1;Set Percentage to +/- 1000%");
		DivideByZeroAction.SetCustomInputIndex(0);

		HighlightAdjacentAlertsGroupSize.Name = "Highlight Adjacent Alerts Minimum Group Size";
		HighlightAdjacentAlertsGroupSize.SetInt(0);
	...
	if (HighlightAdjacentAlertsGroupSize.GetInt() == 0)
	{
		for (int SubgraphIndex = 0; SubgraphIndex < 10; ++SubgraphIndex)
		{
	...
	else if (HighlightAdjacentAlertsGroupSize.GetInt() != 0)
	{
	...
	int HighlightAdjacentAlertsGroupSizeValue = HighlightAdjacentAlertsGroupSize.GetInt();
	...
	for (int BarIndex = sc.UpdateStartIndex; BarIndex < sc.ArraySize; BarIndex++)
	{
		...
		for (int PriceIndex = 0; PriceIndex < NumberOfPricesAtBarIndex; ++PriceIndex)
		{
			...
			unsigned int AdditionalVolumeThresholdValue = AdditionalVolumeThreshold.GetInt();
			...
			{
				bool AllowZeroValueComparesSetting = AllowZeroValueCompares.GetYesNo();
				unsigned int DivideByZeroActionIndex = DivideByZeroAction.GetIndex();
				...
				int PercentThresholdSigned = PercentageThreshold.GetInt();
			...
			if (ConditionMet)
			{
				...
				if(DrawExtensionLines.GetYesNo()
					&& sc.GetBarHasClosedStatus(BarIndex) == BHCS_BAR_HAS_CLOSED)
				{
					sc.AddLineUntilFutureIntersection
						( BarIndex
						, AvailableSubgraphIndex  // LineIDForBar
						, PriceForExtensionLine
						, ExtensionLinePropertiesSubgraph.PrimaryColor
						, ExtensionLinePropertiesSubgraph.LineWidth
						, ExtensionLinePropertiesSubgraph.LineStyle
						, false
...
SCSFExport scsf_LoadDaySessionOnlyAtOpen(SCStudyInterfaceRef sc)
{
	...
	if (r_PreviousCurrentTime != 0.0
		...
		&& CurrentDateTime.GetTime() >= sc.StartTime1
		&& r_PreviousCurrentTime.GetTime() < sc.StartTime1)
	{
	...
	if (r_PreviousCurrentTime != 0.0
		&& !sc.UseSecondStartEndTimes
		&& CurrentDateTime.GetTime() > sc.EndTime1
		&& r_PreviousCurrentTime.GetTime() <= sc.EndTime1)
	{
