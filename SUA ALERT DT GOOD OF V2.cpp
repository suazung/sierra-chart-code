...
SCSFExport scsf_AlertDTWithGoodOF(SCStudyInterfaceRef sc)
{
	...
	for (int i = detectDTWithGoodOF->size() - 1 ; i >= 0; i--)   
	{
		...
		if(currentIndex >= sc.DataStartIndex && sc.Subgraph[0][currentIndex-1] == 1)
		{
			...
			text.Format("GU SELL NOW! at %.4f                                        ", sc.Low[detectDTWithGoodOF->at(i).indexOfGoodOfEndBar] );
			text += "\n";
			text.Append("\n");
			text.AppendFormat("SL at %.4f [%dT]                                        " , sc.High[detectDTWithGoodOF->at(i).indexOfGoodOfStartBar] + sc.TickSize, slInTick );
			text.Append("\n");
			text.AppendFormat("TP at %.4f [%dT]                                        " , sc.Low[detectDTWithGoodOF->at(i).indexOfGoodOfEndBar] - (tp_in_ticks*sc.TickSize), tpInTick );
