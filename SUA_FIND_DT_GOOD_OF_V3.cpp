...
//      ################################################                        ############################################
		...
		for (int i = 0; i < detectDTWithGoodOF->size(); i++)
		{
			...
			if(num_touch_vwap >= 10)  continue;               // 19
...
void simTrade(SCStudyInterfaceRef sc , std::vector<st_DoubleTopWithGoodOF>& detectDTwithGoodSellOF , std::vector<st_SimTrade>& detectTrade , int tp_in_ticks) 
{
	...
	for (int i = 0; i < detectDTwithGoodSellOF.size(); i++)
	{
		...
		//if( i >=1 && pvIndex == detectDTwithGoodSellOF[i-1].indexOfPivotHigh && stIndex == detectDTwithGoodSellOF[i-1].indexOfGoodOfStartBar ) continue ;  // good
		
		// -------------  go next i if have same end index
		//if( i >=1 && entryIndex == detectDTwithGoodSellOF[i-1].indexOfGoodOfEndBar) continue ;  // good
	...
	for(int i = 0 ; i < detectTrade.size() ; i++)
	{
		...
		int num_up_candle_poc_at_low = findHowManyUpCandleWithPocAtLow( sc , endIndex , 9 );
		...
		{
			if(goodSellArray[i] == 1 && goodSellStartIdxArray[i] != pivotIndex  
				&& startIndex > goodSellStartIdxArray[i] /*&& pivotIndex > goodSellStartIdxArray[i]*/) 
			{
				cnt_good_sell++;
				diff_high_of_good_st_bar = sc.PriceValueToTicks( sc.High[startIndex] ) - sc.PriceValueToTicks(sc.High[goodSellStartIdxArray[i]] )  ;
				break ;
			}
		}
		
		
		SCDateTime t0 = sc.BaseDateTimeIn[startIndex];
		SCDateTime t1 = sc.BaseDateTimeIn[endIndex];
		SCString date = sc.DateTimeToString( t1 , FLAG_DT_COMPLETE_DATETIME);
		
		int hmnTouchPV = findHowManyTouchLowPivot( sc ,pivotIndex , startIndex ) ;
		...
		outputFile << ", hmn PV : " << hmnTouchPV ;
		//outputFile << ", dff sec : " << timeDifferenceMilliseconds/1000 ;
		...
		//outputFile << ", num poc : " << num_up_candle_poc_at_low  ;  //  num_up_candle_poc_at_high
		outputFile << ", good buy : " << cnt_good_buy  ; 
		//outputFile << ", good sell : " << cnt_good_sell  ; 
		outputFile << ", diff good sell st bar: " << diff_high_of_good_st_bar  ;    
