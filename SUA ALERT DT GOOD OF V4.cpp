...
SCSFExport scsf_AlertDTWithGoodOF(SCStudyInterfaceRef sc)
{
	   ...
	   sc.GetBarHasClosedStatus(currentIndex-1)==BHCS_BAR_HAS_CLOSED )
	{
		 ...
		 if ( currentIndex != lastProcessedBarIndex  ) // Only send message if it's a new bar
		{
			...
			int uniqNumber = persistentUniqueValue + DT_GOOD_OF_CODE  ;
			...
			textToTelegram.AppendFormat("SL at %.4f [%dT]                                        " , sc.High[idx_of_start_bar] + sc.TickSize, slInTick );
			textToTelegram.Append("\n");
